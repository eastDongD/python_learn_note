...
## py也可以匹配'python'，但是加上^py$就变成了整行匹配，就只能匹配'py'了  即正则表达式的匹配应该是只要字符串有我这些内容就可以
# 所以字符匹配的意思应该是只要字符串从左至右看有一段子串满足正则表达式即匹配成功，但是加上^ $后必须全部满足要求
...
# python中使用正则表达式（用于验证字符串是否合法，切分字符串，提取子串）及重复使用时，为了提高效率的预编译
